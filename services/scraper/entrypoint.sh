...
echo "[scraper] HEVY_API_KEY is set: $([ -n \"$HEVY_API_KEY\" ] && echo 'yes' || echo 'no')"
...
while true; do
    ...
    if python scripts/scrape.py; then
        ...
        echo "[scraper] Scrape failed (expected with test/invalid keys)"
    fi
    ...
    echo "[scraper] Next scrape in ${INTERVAL}s"
    sleep "$INTERVAL"
done
